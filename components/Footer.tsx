import { socialLinks } from '@/lib/constants'

const iconPaths = {
  mail: 'M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z M22,6 L12,13 L2,6',
  linkedin: 'M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z M2 9h4v12H2z M4 6a2 2 0 100-4 2 2 0 000 4z',
  github: 'M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22',
  twitter: 'M23 3a10.9 10.9 0 0 1-3.14 1.53 4.48 4.48 0 0 0-7.86 3v1A10.66 10.66 0 0 1 3 4s-4 9 5 13a11.64 11.64 0 0 1-7 2c9 5 20 0 20-11.5a4.5 4.5 0 0 0-.08-.83A7.72 7.72 0 0 0 23 3z',
}

export default function Footer() {
  const currentYear = new Date().getFullYear()
  
  return (
    <footer className="py-20 border-t border-border bg-background" role="contentinfo">
      <div className="container">
        <div className="text-center">
          <div className="flex flex-wrap justify-center gap-6 mb-8">
            {socialLinks.map(link => (
              <a
                key={link.label}
                href={link.href}
                className="flex items-center gap-2 text-text-secondary hover:text-accent-text transition-colors font-medium touch-target"
                target={link.href.startsWith('http') ? '_blank' : undefined}
                rel={link.href.startsWith('http') ? 'noopener noreferrer' : undefined}
                aria-label={link.ariaLabel}
              >
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  aria-hidden="true"
                >
                  {link.icon === 'mail' && (
                    <>
                      <path d={iconPaths.mail.split(' ')[0]} />
                      <polyline points={iconPaths.mail.split(' ')[1]} />
                    </>
                  )}
                  {link.icon === 'linkedin' && (
                    <>
                      <path d={iconPaths.linkedin.split(' ')[0]} />
                      <rect x="2" y="9" width="4" height="12" />
                      <circle cx="4" cy="4" r="2" />
                    </>
                  )}
                  {link.icon === 'github' && (
                    <path d={iconPaths.github} />
                  )}
                  {link.icon === 'twitter' && (
                    <path d={iconPaths.twitter} />
                  )}
                </svg>
                <span>{link.label === 'Email' ? 'ben@bencastillo.dev' : link.label}</span>
              </a>
            ))}
          </div>
          
          <p className="text-text-muted text-sm">
            © {currentYear} Ben Castillo. Built with Next.js, TypeScript, and Tailwind CSS.
          </p>
        </div>
      </div>
    </footer>
  )
}